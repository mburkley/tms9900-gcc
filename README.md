...
* R0 - reserved for bit shift operations
* R11 - Function return pointer
* R12 - Not currently used but reserved for either CRU operations or static chain
pointer
* R14 - Base pointer for stack frames
* R15 - Stack pointer

The compiler will use R1-R10 and R13 as general registers.  Parameters are
passed in registers starting from R1.

The compiler does not have any ability to emit any CRU instructions (STCR,LDCR,SB,SBO,TB).  It is
safe to modify R12 as the compiler knows it is reserved.

The compiler no longer saves R13,R14 and R15 when entering a function.  If you are writing code that
will be invoked through a BLWP instruciton, then you must save these regisers
