  ...
  ""
  {
    ...
    switch(GET_CODE(operands[1]))
    {
      ...
      case MEM:
        ...
        output_asm_insn("mov  %0, %2", args);
        output_asm_insn("mov  %1, %3", args);
        // return("mov  %0, %2");
        // return("mov  %1, %3");
        // TODO move high word
        break;
...
;;-------------------------------------
; byte mul to word mul
;
(define_insn "mulqihi3"
   [(set (match_operand:HI 0 "register_operand" "=r")
        (mult:HI (zero_extend:HI (match_operand:QI 1 "register_operand" "r"))
                 (zero_extend:HI (match_operand:QI 2 "register_operand" "rRQ>"))))]
  ""
  {
    tms9900_debug_operands ("mulqihi3", operands, 3);
    output_asm_insn("mov  %0, %1", operands);
    output_asm_insn("mpy  %0, %2", operands);
    return("");

    #if 0
    // Attempt to expand qihi3 to hisi3
    emit_insn(gen_mulhisi3(operands[0], operands[1], operands[2]));
    tms9900_inline_debug ("; mulqihi3 : expand done\n");
    DONE;
    
    // Attempt to expand by emitting extends
    emit_insn(gen_extendqihi2(operands[1], operands[1]));
    emit_insn(gen_extendqihi2(operands[2], operands[2]));
    // emit_move_insn(operands[3],gen_rtx_ZERO_EXTEND(SImode, operands[0]));
    emit_insn(gen_mulhisi3(operands[0], operands[1], operands[2]));
    emit_move_insn(operands[0], gen_lowpart(HImode, operands[3]));
    // TODO restore op1 and op2?
    DONE;
...
;;-------------------------------------
; This is the only native multiplication on the tms9900.  HIxHI=SI
...
(define_insn "mulhisi3"
  ...
  ""
  {
    ...
    // assume we have been passed 3 operates, 1 SI and 2 HI.  op0 
    // may be in the same register as op1 ?
    tms9900_inline_debug ("; mulhisi3 : reg(op0)=%d, reg(op1)=%d reg(op2)=%d\n",
                          REGNO(operands[0]),
                          REGNO(operands[1]),
                          REGNO(operands[2]));
    rtx args[4];
    // args[0] = gen_rtx_REG (HImode, REGNO (operands[0]));  /* High word of result */
    args[0] = gen_highpart (HImode, operands[0]);
    // args[1] = gen_rtx_REG (HImode, REGNO (operands[0]) + REGS_PER_WORD);  /* Low word of result */
    args[1] = gen_lowpart (HImode, operands[0]);
    args[2] = operands[1];
    args[3] = operands[2];

    output_asm_insn("clr  %0", args);
    output_asm_insn("mov  %1, %2", args);
    output_asm_insn("mul  %0, %3", args);
...
(define_expand "mulhi3"
  ...
  ""
  {
    ...
    emit_insn(gen_mulhisi3(operands[3], operands[1], operands[2]));
