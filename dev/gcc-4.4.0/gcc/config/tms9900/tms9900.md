...
;;
;; Byte variants are absolutely needed and implemented, but legacy might generate the above?
;; However, they are NOT currently checking for shifts of 0 or shifts >8 as noted below.
;;
;; NOTE-1 : On the TMS9900 a shift count of 0 is interpreted as
;; shift by 16, so test for 0 and jump over the shift if true.  Do the same in
;; all other shift by register insns as well.
;;
;; NOTE-2 : 32-bit shift operations are not implemented as insns at the moment
;; because the sequence of instructions would be quite lengthy.  There are library
;; functions in lib1funcs.asm to do 32-bit shift ops.  Unfortunately when building
;; for speed optimisation, the compiler does not use the library and instead tries
;; to construct its own 32-bit shift using 16-bit ops.  This works except when
...
;;-------------------------------------------------------------------
;; Arithmetic shift left, QImode
;;
;; A QImode value lives in the HIGH byte of its register.  The TMS9900 shift
;; opcodes operate on a full 16-bit word, so the byte must first be moved into
;; the low half.  For a SIGNED source this repositioning rides along on the
;; sign_extend (sra ,8), which the optimiser cannot delete because the sign
;; bits are live.  For an UNSIGNED source the only thing that would reposition
;; the byte is the zero_extend (srl ,8), but the optimiser treats that extend
;; as dead under a left shift and deletes it, leaving the byte stranded in the
;; high half.  To guarantee correct positioning regardless of signedness we
;; widen explicitly here and perform the shift in HImode, then truncate back.
;;-------------------------------------------------------------------
(define_expand "ashlqi3"
  ...
  ""
  {
    ...
    /* Force the byte down into the low half with a committed widening insn.
       Using zero_extend here positions the byte (srl ,8) AND clears the
       high byte; because this insn is emitted as part of the shift sequence
       rather than as a standalone promotion, it is not eligible for the
       dead-extend deletion that strands the unsigned byte in ashlhi3. */
    emit_insn (gen_zero_extendqihi2 (wide, operands[1]));

    /* Shift in HImode where the value is now correctly positioned. */
    emit_insn (gen_ashlhi3 (wide, wide, operands[2]));

    /* Truncate the HImode result back to the QImode destination (byte returns
       to the high half via the movqi/trunc path). */
    emit_insn (gen_trunchiqi2 (operands[0], wide));
    DONE;
  }
)

;;-------------------------------------------------------------------
;; Logical shift right, QImode
;;
;; Same problem and fix as ashlqi3.  A QImode value lives in the HIGH byte of
;; its register; the TMS9900 shift opcodes work on a full 16-bit word, so the
;; byte must be moved into the low half first.  For an UNSIGNED source the only
;; reposition would ride on the zero_extend (srl ,8), but the optimiser deletes
;; that extend as dead, stranding the byte in the high half.  Widen explicitly
;; here as a committed insn so the reposition cannot be removed, shift in
;; HImode, then truncate back.  Zero-extend is also semantically required for a
;; logical right shift so that zeros, not sign bits, enter from the top.
;;-------------------------------------------------------------------
(define_expand "lshrqi3"
  ...
  ""
  {
    ...
    /* Committed widening: srl ,8 positions the byte in the low half and clears
       the high byte.  Emitted as part of the shift sequence so it is not
       eligible for the dead-extend deletion that strands the unsigned byte. */
    emit_insn (gen_zero_extendqihi2 (wide, operands[1]));

    /* Logical right shift in HImode, where the value is now correctly placed
       and the cleared high byte ensures zeros shift in from the top. */
    emit_insn (gen_lshrhi3 (wide, wide, operands[2]));

    /* Truncate the HImode result back to the QImode destination. */
    emit_insn (gen_trunchiqi2 (operands[0], wide));
    DONE;
  }
)
...
(define_expand "divmodqi4"
  ...
  ""
  {
    ...
    /* Find and store sign of quotient and sign of remainder (same as sign of dividend) */
    rtx sign_quotient = gen_reg_rtx (HImode);
    rtx sign_remainder = gen_reg_rtx (HImode);
    emit_insn(gen_extendqihi2 (operands[1], sign_quotient));
    emit_move_insn(sign_remainder, sign_quotient);
    emit_insn(gen_extendqihi2 (operands[2], operands[2]));
    emit_insn(gen_xorhi3(sign_quotient, sign_quotient, operands[2]));
    ...
    emit_insn(gen_divfixuphi2(quotient, sign_quotient));
    ...
    emit_insn(gen_divfixuphi2(remainder, sign_remainder));

    /* Truncate remainder to byte */
    emit_insn(gen_trunchiqi2 (operands[3], remainder));
...
(define_expand "divmodhi4"
  ...
  ""
  {
    ...
    rtx sign_quotient = gen_reg_rtx (HImode);
    rtx sign_remainder = gen_reg_rtx (HImode);
    emit_move_insn(sign_quotient, operands[1]);
    emit_move_insn(sign_remainder, operands[1]);
    emit_insn(gen_xorhi3(sign_quotient, sign_quotient, operands[2]));
    ...
    /* Correct sign of quotient */
    emit_insn(gen_divfixuphi2(operands[0], sign_quotient));

    /* Correct sign of remainder */
    emit_insn(gen_divfixuphi2(operands[3], sign_remainder));
...
(define_insn "divfixuphi2"
  ...
  ""
  {
    ...
    // output_asm_insn("jgt  $+4", operands);
    output_asm_insn("neg  %0", operands);
    return("");
  }
  [(set_attr "length" "6")])
