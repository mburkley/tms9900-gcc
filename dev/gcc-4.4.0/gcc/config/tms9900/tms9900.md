...
;;    - I'm avoiding clobbers in expands and declaring temp rtx regs inside code instead
...
(define_insn "movqi"
  [(set (match_operand:QI 0 "nonimmediate_operand" "=rR>,rR>,Q,  Q,r,r")
        (match_operand:QI 1 "general_operand"      "rR>, Q,  rR>,Q,i,OM"))]
  ""
  {
...
(define_insn "movhi"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=rR>,rR>,Q,  Q,r,r")
	(match_operand:HI 1 "general_operand"      "rR>, Q,  rR>,Q,i,OMJ"))]
  ""
  {
    ...
    case 4:
        return("li   %0, %1");
    case 5:
        // TODO not working, matches alt 4 for op1==0
        val = INTVAL(operands[1]);
...
(define_expand "movsi"
  [(set (match_operand:SI 0 "nonimmediate_operand" "=rR>,rR>,Q,  Q,r,r")
	(match_operand:SI 1 "general_operand"      "rR>, Q,  rR>,Q,i,OM"))]
  ""
  {
    tms9900_debug_operands ("movsi", operands, 2);
    tms9900_inline_debug ("; movsi alt=%d\n", which_alternative);

    /*  Alternates don't seem to work in expands? */
    if (GET_CODE (operands[1]) == CONST_INT)
    {
      rtx val_hi = gen_rtx_CONST_INT (HImode, (INTVAL(operands[1])>>16));
      rtx val_lo = gen_rtx_CONST_INT (HImode, (INTVAL(operands[1])&0xffff));
      ...
      {
        tms9900_inline_debug ("; movsi op0 code is %s\n",
            GET_RTX_NAME(GET_CODE(operands[0])));

        /*  We can't move a constant directly to memory.  Create an intermediate
         *  reg and move that */
        rtx temp = gen_reg_rtx(HImode);
        tms9900_inline_debug ("; movsi hi-const to reg\n");
        emit_move_insn(temp, val_hi);
        tms9900_inline_debug ("; movsi reg to hi-dst\n");
        emit_move_insn(gen_highpart (HImode, operands[0]), temp);
        tms9900_inline_debug ("; movsi lo-const to reg\n");
        emit_move_insn(temp, val_lo);
        tms9900_inline_debug ("; movsi reg to lo-dst\n");
        emit_move_insn(gen_lowpart (HImode, operands[0]), temp);
        tms9900_inline_debug ("; movsi done\n");
      }
      else
      {
        /*  Move the const directly to the dest */
        emit_move_insn(gen_highpart (HImode, operands[0]), val_hi);
        emit_move_insn(gen_lowpart (HImode, operands[0]), val_lo);
      }
    }
    else
    {
        /*  Non immediate mov */
        emit_move_insn(gen_highpart (HImode, operands[0]),
...
(define_insn "truncsihi2"
  [(set (match_operand:HI 0 "register_operand" "=rR>,Q")
	(truncate:HI (match_operand:SI 1 "register_operand" "0,0")))]
  ""
  {
