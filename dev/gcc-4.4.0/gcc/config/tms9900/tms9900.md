...
(define_insn "tms9900_movqi"
  ...
  ""
  {
    ...
    /*  If a byte swap is needed in the dest, then do mov instead of movb to
     *  copy the whole 16-bit value
     */
    if (postByteSwap)
    {
      output_asm_insn ("mov  %1, %0 ; tms movqi", operands);
      output_asm_insn ("swpb %0 ; movqi byte correct", operands);
      printf ("swpb post movqi byte correct");
    }
    else
...
(define_insn "andhi3"
  ...
  ""
  {
     ...
     *
     * We can ignore the return value from correct byte order because the target
     * is a HI so the case where a post byte swap is required should never
     * arise.
     */
