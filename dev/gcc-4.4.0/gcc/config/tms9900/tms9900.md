...
(define_insn "tms9900_cmphi"
  ...
  ""
  {
    tms9900_debug_operands ("tms9900_cmphi", insn, operands, 2);
...
(define_expand "cmphi"
  ...
  ""
  {
    tms9900_debug_operands ("cmphi", NULL_RTX, operands, 2);

    /*  If we are comparing a memory value to an immediate, it is cheaper to
     *  compare to a literal than load an immediate to a scratch reg and compare to the
     *  reg */
    if (!REG_P (operands[0]) && CONST_INT_P (operands[1]))
    {
      tms9900_inline_debug ("; cmphi replace const with label\n");
      operands[1] = force_const_mem (HImode, GEN_INT (INTVAL (operands[1]) & 0xffff));
    }

    emit_insn (gen_tms9900_cmphi (operands[0], operands[1]));
    DONE;
  }
)
...
;;-------------------------------------
;
; TODO expand to general, not just memory
;
; Replace:
;   mov @x, r1
;   cmp @y, r1
...
; NOTE - byte immediate values are replaced with a label to a value in a
;        constant pool. The exception for compare to zero has been removed as
;        adding a clr instruction is longer than storing a zero byte in the constant pool

;  TODO bug : expand cmpqi causes premature abort of compilation of
;  conio_cgetc.c.  Doesn't happen if this expand is reverted.
;
; (define_expand "cmpqi"
(define_insn_and_split "cmpqi"
  ...
  [(set (cc0)
	(compare (match_dup 0)
		 (match_dup 1)))]
  {
    // tms9900_debug_operands ("cmpqi", NULL_RTX, operands, 2);

    // if (CONST_INT_P (operands[1]))
    // {
      int val = INTVAL (operands[1]) & 0xff;
    //   tms9900_inline_debug ("; cmpqi split replace const with label\n");
    //   printf("val=%x\n",val);
      operands[1] = force_const_mem (QImode, GEN_INT (val));
    // }
    // else operands[2] = operands[1];
  
// printf("MGB force_const_mem : ");
// print_inline_rtx (stdout, operands[1], 0);
// printf("\n");

    // emit_insn (gen_tms9900_cmpqi (operands[0], op));
    // DONE;
  }
)

;;-------------------------------------
; Byte compares to immediates are problematic so they are not allowed.  Compares
; to immediates will be handled by the cmpqi expand which will replace the
; immediate.
;
(define_insn "haha_cmpqi"
  [(set (cc0)
	(compare (match_operand:QI 0 "nonimmediate_operand" "rR>,Q,  rR>,Q")
		 (match_operand:QI 1 "nonimmediate_operand" "rR>,rR>,Q,  Q")))]
  ""
  {
    tms9900_debug_operands ("tms9900_cmpqi", insn, operands, 2);
    return "cb   %0, %1";
  }
...
(define_insn "tms9900_clrqi"
  ...
  ""
  {
    tms9900_debug_operands ("tms9900_clrqi", insn, operands, 1);
    return "clr  %0";
  }
  [(set_attr "length" "2")])
...
(define_insn "tms9900_setoqi"
  ...
  ""
  {
    tms9900_debug_operands ("tms9900_setoqi", insn, operands, 1);
    return "seto %0";
  }
  [(set_attr "length" "2")])
...
(define_expand "movqi"
  ...
  ""
  {
    ...
    if (CONST_INT_P (operands[1]))
    {
      if (REG_P (operands[0]) && (INTVAL (operands[1]) & 0xff) == 0)
      {
        emit_insn (gen_tms9900_clrqi (operands[0]));
        DONE;
      }

      if (REG_P (operands[0]) && (INTVAL (operands[1]) & 0xff) == 0xff)
      {
...
(define_insn "tms9900_movqi"
  ...
  ""
  {
    ...
    // Could this check be moved to the expand?  Probably not.  The trunc would
    // probably get eliminated before the code gets emitted

    bool postByteSwap = tms9900_correct_byte_order (insn, operands);

    output_asm_insn ("movb %1, %0 ; tms movqi", operands);

    if (postByteSwap)
      output_asm_insn ("swpb %0 ; movqi byte correct", operands);

    tms9900_inline_debug ("; movqi done\n");
    return "";
  }
  [(set_attr "length" "2,4,4,6")])
...
(define_insn "*zero_extendqihi2_subreg"
  ...
  ""
  {
    ...
    {
      output_asm_insn ("movb %1, %0 ; zext sub", operands);
    }
...
(define_insn "zero_extendqihi2"
  ...
  ""
  {
    ...
    if (which_alternative > 0)
    {
      output_asm_insn ("movb %1, %0 ; zext", operands);
    }
    output_asm_insn ("srl  %0, 8", operands);
    return("");
  }
...
(define_insn "extendqihi2"
  ...
  ""
  {
    ...
    if (which_alternative > 0)
    {
      output_asm_insn ("movb %1, %0 ; ext", operands);
    }
...
;;-------------------------------------
(define_insn "subhi3"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=rR>,Q,rR>,Q")
	(minus:HI (match_operand:HI 1 "nonimmediate_operand" "0,0,0,0")
		  (match_operand:HI 2 "nonimmediate_operand" "rR>,rR>,Q,Q")))]
  ""
  {
...
(define_insn "*movqi_for_initializer"
  ...
  ""
  {
    ...
    {
      output_asm_insn("movb %1, %0 ; init", operands);
    }
    else if(INTVAL(operands[2]) == 0)
    {
      output_asm_insn("swpb %1", operands);
      output_asm_insn("movb %1, %0 ; init swp", operands);
    }
   return("");
  }
  [(set_attr "length" "2,4")])
