...
;;-------------------------------------
 (define_insn "tstqi"
   [(set (reg:CC CC_REGNUM)
         (compare:CC (match_operand:QI 0 "nonimmediate_operand" "rR,Q")
                     (const_int 0)))]
   ""
   {
...
;;-------------------------------------
;; There are a lot of alternatives here but this is to enable as many modes of
;; use as possible.  It isn't possible to make this insn an expand or a split since
;; it is matched in later stages when it is expanded by comparison operators.
;; Comparing a memory location to a constant requires use of a clobber register to
;; load an immediate

;;-------------------------------------
(define_insn "cmphi"
  [(set (reg:CC CC_REGNUM)
        (compare:CC (match_operand:HI 0 "nonimmediate_operand" "rR,rR,Q,Q,rR,Q,r,R,Q")
                    (match_operand:HI 1 "general_operand"      "rR,Q,rR,Q,O, O,i,i,i")))
  ...
  ""
  {
    ...
    if (which_alternative < 4)
      ...
      // Comparing to const_int 0 is done by emitting mov
      return ("mov  %0, %0");
    else if (which_alternative == 6)
      return ("ci   %0, %1");
    else
    {
      ...
      output_asm_insn("c    %0, %2",  operands);
      return "";
    }
  }
  [(set_attr "length" "2,4,4,6,2,6,4,6,8")])


;;-------------------------------------

; TEST if is the clobber that breaks the match - nope
; TEST if needs reload_compelted - nope
; TEST if operands must match splut

(define_insn "cmpqi"
  [(set (reg:CC CC_REGNUM)
        (compare:CC (match_operand:QI 0 "nonimmediate_operand" "rR,rR,Q, Q,rR,Q,r,R,Q")
                    (match_operand:QI 1 "general_operand"      "rR,Q, rR,Q,O, O,i,i,i")))
  ...
  ""
  {
    ...
    if (which_alternative < 4)
      ...
      // Comparing to const_int 0 is done by emitting movb
      return ("movb %0, %0");
    else
    {
      // Compare only one byte of the reg by loading to a clobber and then doing
      // a byte compare.  This is more efficient than doing ANDI and then CI.
      int val = INTVAL(operands[1]) & 0xff;
      operands[1] = GEN_INT(val<<8);
      output_asm_insn("li   %2, %1",  operands);
      output_asm_insn("cb   %0, %2",  operands);
      return "";
    }
  }
  [(set_attr "length" "2,4,4,6,2,6,6,6,8")])

; TEST if const_int needs a sep compare

; (define_insn "*cmpqi_const"
;   [(set (reg:CC CC_REGNUM)
;         (compare:CC (match_operand:QI 0 "nonimmediate_operand" "r,R,Q")
;                     (match_operand:QI 1 "const_int_operand"    "i,i,i")))
;     (clobber (match_operand:QI 2 "register_operand" ""))]
;   ""
;   {
;     tms9900_debug_operands ("cmpqi_const", operands, 2);
;       // Compare only one byte of the reg by loading to a clobber and then doing
;       // a byte compare.  This is more efficient than doing ANDI and then CI.
;       int val = INTVAL(operands[1]) & 0xff;
;       operands[1] = GEN_INT(val<<8);
...
;; compare to zero.  Do we need a CCNZmode?
;; TODO can split use mode iterators?
...
(define_insn "*mov_hi_zero"
  [(set (match_operand:HI 0 "nonimmediate_operand" "")
        (const_int 0))]
  ""
  "clr %0")

(define_insn "*mov_hi_minus_one"
  [(set (match_operand:HI 0 "nonimmediate_operand" "")
        (const_int -1))]
  ""
  "seto %0")
...
; emit will come from an insn that clobbers CC
...
; Define an insn that moves a byte, which on tms9900 has a side effect of comparing the
; source to zero
(define_insn "movqi<cc_cc>"
...
(define_insn "*movhi_nocc"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=rR>,rR>,Q,  Q,r, r")
        (match_operand:HI 1 "general_operand"      "rR>, Q,  rR>,Q,OM,i"))
   (clobber (reg:CC CC_REGNUM))]
  ""
  {
...
; TODO could be a split?  Output mov and lshift as separates might be better
...
;                         (match_operand:QHint 2 "reg_or_0_operand")])
;                       (label_ref (match_operand 1 "" ""))  <- should be 3?
;                       (pc)))]
;   "")
...
(define_insn_and_split "cbranch<mode>4"
  [(set (pc)
        (if_then_else (match_operator 0 "ordered_comparison_operator"
                       [(match_operand:QHint 1 "nonimmediate_operand" "g")
                        (match_operand:QHint 2 "general_operand" "g")])
...
;; TODO should be R,Q ?  "b rx" doesn't make sense
