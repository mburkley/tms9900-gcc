...
; TEST if expand and then insn works
...
(define_expand "cmp<mode>"
  ...
  ""
  {
    tms9900_debug_operands ("cmp<mode>", operands, 1);
    rtx comp = operands[1];
    ...
    if (GET_CODE (operands[1]) == CONST_INT)
    {
      ...
      {
        comp = gen_reg_rtx (GET_MODE (operands[0]));
        emit_move_insn (comp, operands[1]);
      }
    }

    if (GET_MODE (operands[0]) == QImode)
      emit_insn (gen_cmpqi_reg (operands[0], comp));
    else
      emit_insn (gen_cmphi_reg (operands[0], comp));
  })

; TODO create versions of this with diff operamds, reg, immed,etc so as not to
; require clobbers for all.  Suspect clobber with reload is broken as no regs
; avail

(define_insn "cmpqi_reg"
  [(set (reg:CC CC_REGNUM)
        (compare:CC (match_operand:QI 0 "nonimmediate_operand" "rR,rR,Q, Q")
                    (match_operand:QI 1 "nonimmediate_operand" "rR,Q, rR,Q")))]
  ""
  "cb %0 %1"
  [(set_attr "length" "2,4,4,6")])

(define_insn "cmphi_reg"
  [(set (reg:CC CC_REGNUM)
        (compare:CC (match_operand:HI 0 "nonimmediate_operand" "rR,rR,Q, Q")
                    (match_operand:HI 1 "nonimmediate_operand" "rR,Q, rR,Q")))]
  ""
  "c  %0 %1"
  [(set_attr "length" "2,4,4,6")])
...
;  truncate so I am assuming we don't care about unused bits.
;;-------------------------------------
