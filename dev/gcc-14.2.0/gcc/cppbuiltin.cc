...
#ifdef TMS9900
  /* Let target code query the tms9900 backend patch level, eg to work
     around a bug fixed in a specific patch release.  */
  {
    int patch_major = 0, patch_minor = 0;
    sscanf (TMS9900_REVISION, "%d.%d", &patch_major, &patch_minor);
    cpp_define_formatted (pfile, "__TMS9900_PATCH_MAJOR__=%d", patch_major);
    cpp_define_formatted (pfile, "__TMS9900_PATCH_MINOR__=%d", patch_minor);
  }
#endif
}
