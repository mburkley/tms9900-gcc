...
;
;  MGB: that history is preserved above for context, but this is no longer a
; plain define_insn: a constant-address call target, e.g. (mem:HI (const_int
; N)), is bit-identical to a data load at N and CSE was observed forwarding a
; data value into a call target.  tms9900_fixup_call_target() marks such call
; MEMs volatile so they can't be forwarded, but that has to happen while the
; RTL is still being built, which needs a define_expand.  Re-tested here and
; the segfault this comment describes does not reproduce, so both call and
; call_value are now the usual define_insn/define_expand pair.
...
(define_insn "tmps9900_cmp<mode>_zero"
  ...
  ""
  {
    tms9900_debug_operands ("tms9900_cmp<mode>_zero", operands, 1);
...
;; This splits all the integer moves: QI and HI modes as well as
...
(define_insn "*tms9900_movqi_zero<cc_cc>"
  [(set (match_operand:QHint 0 "nonimmediate_operand" "=r,R>,Q")
        (const_int 0))
   (clobber (reg:CC CC_REGNUM))]
  ""
  {
      // if (<QHint:mname> == QImode && which_alternative > 0)
      if (which_alternative > 0)
      {
        output_asm_insn ("clr  r0", operands);
        output_asm_insn ("movb r0, %0", operands);
      }
      else
...
(define_insn "*tms9900_movhi_zero<cc_cc>"
...
; TODO fix for bytes
(define_insn "*tms9900_mov<mode>_minus_one<cc_cc>"
  [(set (match_operand:QHint 0 "nonimmediate_operand" "=rR>,Q")
        (const_int -1))
   (clobber (reg:CC CC_REGNUM))]
  ""
...
(define_insn "tms9900_movhi_const<cc_cc>"
...
(define_insn "tms9900_movqi_const<cc_cc>"
...
(define_insn "tms9900_mov<mode><cc_cc>"
...
(define_insn "movsi"
  ...
  ""
  {
    ...
    else
    {
      ...
      /* When both operands are hard registers, a register pair move
         (eg [R2,R3] -> [R3,R4]) can have its low half overlap the
         high half of the destination.  Moving high-word-first would
         then clobber the source's low word before it's read.  Detect
         that specific overlap and move low-word-first instead; any
         other case (including the common non-overlapping case) is
         safe with the usual high-word-first order. */
      if (REG_P (operands[0]) && REG_P (operands[1]) &&
          REGNO (args[0]) == REGNO (args[3]))
      {
...
(define_insn "zero_extendhisi2<cc_cc>"
...
;;
;; B does not dereference its source operand the way ordinary
;; instructions do: for register indirect (*Rn) it uses Rn's own value
;; as the new PC, and for indexed/symbolic (@sym(Rn)) it uses the
;; computed address itself, not the value stored there.  So a computed
;; goto through a table of label pointers (eg "goto *table[i]", GCC PR
;; tms9900-gcc#74) must not branch directly against a memory operand --
;; that would jump to the table entry's address instead of the label it
;; points to.  Load the target into a scratch register with an ordinary
;; (dereferencing) mov first, matching how "tablejump" below already
;; does this correctly.
(define_insn "indirect_jump"
  ...
  ""
  {
    ...
    output_asm_insn("b    *%1 ; indir", operands);
    return "";
  }
  [(set_attr "length" "4,6")])
...
;; NOTE: this used to special-case a constant shift count of 2 (the "O"
;; alternative, matched below) to emit nothing, on the theory that the
;; compiler could only ever generate that as a spurious artifact of
;; expanding a 32-bit shift.  That's wrong -- a constant shift-by-2 is
;; a completely ordinary HImode operation (eg it shows up multiplying
;; an array index by 4 as part of scaling by a struct size that's a
;; multiple of 4), and silently dropping it corrupted every one of
;; those.  A constant 2 needs exactly the same "sla %0,%2" as any
;; other constant shift count, so alternative "O" is now handled
;; identically to alternative "i" below.
;;-------------------------------------------------------------------
(define_insn "ashlhi3"
  ...
  ""
  {
    ...
    if (which_alternative < 2)
    {
      ...
      output_asm_insn("andi r0, >F",  operands);  /* reduce to count mod 16: hardware treats an
                                                     R0 count of literal 0 as âshift by 16â, not
                                                     âshift by 0â, which is wrong whenever the
                                                     original count is a nonzero multiple of 16
                                                     (eg 16 itself, as part of a wider shift). */
      output_asm_insn("jeq  $+4",    operands);
      output_asm_insn("sla  %0, 0",  operands);
    }
    else if (which_alternative == 2)
    {
      /* Constraint O only ever matches the literal constant 2, never
         0, so this is always safe as a direct immediate shift. */
      output_asm_insn("sla  %0, %2", operands);
    }
    else if (CONST_INT_P (operands[2]) && INTVAL (operands[2]) == 0)
    {
      /* True no-op.  A hardware-encoded "sla %0,0" does not mean
         shift by zero here -- it means take the shift count from
         the low 4 bits of R0, which GCC has no model of, so a
         literal 0 must never reach the sla below. */
    }
    else
    {
      output_asm_insn("sla  %0, %2", operands);
...
(define_insn "ashlqi3"
  ...
  ""
  {
    ...
    {
      output_asm_insn("mov  %2, r0",  operands);
      output_asm_insn("andi r0, >F",    operands);  /* see ashlhi3 for why this is needed */
    ...
    else if (CONST_INT_P (operands[2]) && INTVAL (operands[2]) == 0)
    {
      /* True no-op.  A hardware-encoded "sla %0,0" does not mean
         shift by zero here -- it means take the shift count from
         the low 4 bits of R0, which GCC has no model of, so a
         literal 0 must never reach the sla below.  (Also must skip
         the andi: with no shift actually happening, it would wrongly
         clear the lower byte of an otherwise-untouched value.) */
    }
    else
    {
      output_asm_insn("andi %0, >FF00", operands);
...
(define_insn "ashr<mode>3"
  ...
  ""
  {
    ...
    {
      /* True no-op.  A hardware-encoded "sra %0,0" does not mean
         shift by zero here -- it means take the shift count from
         the low 4 bits of R0, which GCC has no model of, so a
         literal 0 must never reach the sra below. */
    }
    else
    {
      output_asm_insn("sra  %0, %2", operands);
...
(define_insn "lshr<mode>3"
  ...
  ""
  {
    ...
    {
      /* True no-op.  A hardware-encoded "srl %0,0" does not mean
         shift by zero here -- it means take the shift count from
         the low 4 bits of R0, which GCC has no model of, so a
         literal 0 must never reach the srl below. */
    }
    else
    {
      output_asm_insn("srl  %0, %2", operands);
...
(define_insn "andhi3<cc_cc>"
  ...
  ""
  {
    ...
    if(which_alternative == 4)
    {
      /* Constraint OM matches 2, -2 and -1, not just the two special
         cases below (0 and all-ones) -- eg -2 lands here with
         val==0xfffe, matching neither, so it needs the same general
         "andi" as alternative 5 or the mask is silently never
         applied. */
      if(val == 0)
      {
        /* Result will be zero */
  ...
  [(set_attr "length" "6,8,8,10,4,4,6,8")])
...
(define_insn "andqi3<cc_cc>"
  ...
  ""
  {
    ...
    {
      /* Constraint OM matches 2, -2 and -1, not just the two special
         cases below (0 and all-ones-in-byte) -- eg -2 lands here with
         val==0xfe00, matching neither, so it needs the same general
         "andi" as alternative 5 or the mask is silently never
         applied. */
      if(val == 0)
        output_asm_insn("clr %0", operands);
...
(define_expand "divmodqi4"
  ...
  ""
  {
    ...
    /* C99: the remainder always takes the dividend's sign, regardless of
       the divisor's sign, so track it separately from the quotient sign. */
    rtx dividend_sign = gen_reg_rtx (HImode);
    emit_insn(gen_extendqihi2 (dividend_sign, operands[1]));
    ...
    /* tmp is now a non-negative magnitude, but abs(-32768) overflows back to
       0x8000 in 16 bits, so a sign-extend here would re-introduce a negative
       32-bit value.  Zero-extend so udivmodsihi3 always sees a true unsigned
       magnitude. */
    emit_insn(gen_zero_extendhisi2 (dividend, tmp));
    ...
    emit_insn(gen_divfixuphi2(remainder, dividend_sign));
...
(define_expand "divmodhi4"
  ...
  ""
  {
    ...
    /* C99: the remainder always takes the dividend's sign, regardless of
       the divisor's sign, so track it separately from the quotient sign. */
    rtx dividend_sign = gen_reg_rtx (HImode);
    emit_move_insn(dividend_sign, operands[1]);
    ...
    /* Correct sign of remainder */
    emit_insn(gen_divfixuphi2(operands[3], dividend_sign));
...
;;-------------------------------------
(define_insn "udivmodsihi3<cc_cc>"
  [(set (match_operand:SI 0 "register_operand" "=r,r")
        (ior:SI
          (ashift:SI
            (zero_extend:SI
...
;; Negate operand 0 in place iff operand 1 (the sign register, read-only
;; here) is negative.  The TMS9900 has no direct "jump if >= 0", so the
;; sign test is done on a scratch copy of operand 1 inverted first (NOT
;; flips which side of zero a value falls on), leaving operand 1 itself
;; untouched.
;;
;; Operand 0 is read (via match_dup, for the "leave unchanged"/negate
;; choice) as well as written, so its constraint must be "+r", not
;; "=r".  With a bare "=r", reload is told operand 0 is a pure write
;; with no required incoming value, so it's free to allocate it an
;; arbitrary hard register unconnected to whatever previously computed
;; the value we're meant to be correcting -- silently discarding the
;; real input.  That single-character bug only showed up once a second
;; caller existed (the remainder-sign fix, alongside the pre-existing
;; quotient-sign one): reload's register choice for the quotient case
;; happened to line up by luck, but for the remainder case it picked a
;; register with no producer, and DCE then deleted the division whose
;; result was never actually being read.
(define_insn "divfixuphi2"
  [(set (match_operand:HI 0 "register_operand" "+r")
        (if_then_else:HI (lt:HI (not:HI (match_operand:HI 1 "register_operand" "r"))
                                 (const_int 0))
           (match_dup 0) (neg:HI (match_dup 0))))
   (clobber (match_scratch:HI 2 "=r"))
   (clobber (reg:CC CC_REGNUM))]
  ""
  {
    tms9900_debug_operands ("divfixuphi2", operands, 2);
    output_asm_insn("mov  %1, %2", operands);
    output_asm_insn("inv  %2", operands);
