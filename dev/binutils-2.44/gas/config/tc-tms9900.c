...
tms9900_md_end (void)
{
  /* Nothing to do: output_file_create() already sets the arch/mach on
     stdoutput using TARGET_ARCH/TARGET_MACH (see tc-tms9900.h), and by
     the time md_end is called stdoutput has already been closed.  */
}
...
void tms9900_cons_fix_new (fragS *frag_p, int offset, int nbytes,
			    expressionS *exp,
